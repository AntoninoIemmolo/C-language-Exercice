...
void Ordina (T_TRATTA** Vet_ord,int campo);
...
T_TRATTA*** ricerca_tratta(T_TRATTA** PuntIn,T_TRATTA** PuntFin,char target[],T_TRATTA*** VetPunt,int cont);


int main(int argc, const char * argv[]) {
    T_TRATTA Vet_Tratte[MaxDati]={0}, t,*Vet_ord[MaxDati],**VetPunt[2]={NULL,NULL},**PuntaMultiOrd[5]; 
    int N_Tratte=0, Codice_Operazione = 0,Codice_Operazione2=-1,c=0;
    FILE* Fin,*Fout;
	...
	if(Codice_Operazione2==5){
		...
		for (int i=0;i<MaxDati;i++){
			...
			printf("(Cod_trat = %s, Part = %s, Dest = %s, Data = %s, Ora Part = %s, Ora Arrivo = %s, Rit = %d)\n",Vet_Tratte[i].Cod_trat,Vet_Tratte[i].Part,Vet_Tratte[i].Dest,Vet_Tratte[i].Data,Vet_Tratte[i].Ora_Part,Vet_Tratte[i].Ora_Arri,Vet_Tratte[i].Rit);
		...
		scanf("%s",stringTarg);
		ricerca_tratta(Vet_ord,&Vet_ord[MaxDati-1],stringTarg,VetPunt,0);
	...
	else if(Codice_Operazione2==7){
		
//Si finisca il caso multi ordinamento
//Si controlli la stabilità
//poi ricontrolla tutto e hai finito
//è il te delle 3:41 am che te lo ordina
	}
	else Ordina (Vet_ord,Codice_Operazione2);
...
T_TRATTA*** ricerca_tratta(T_TRATTA** PuntIn,T_TRATTA** PuntFin,char target[],T_TRATTA*** VetPunt,int cont ){
	cont++;
	int i=0,retVal;	
	T_TRATTA** Punt=PuntIn+(MaxDati/(2*cont));
	...
	if((int)((*Punt)->Part)[retVal]<(int)target[retVal])
		return ricerca_tratta(Punt,PuntFin,target,VetPunt,cont);
	if((int)((*Punt)->Part)[retVal]>(int)target[retVal])
		return ricerca_tratta(PuntIn,Punt,target,VetPunt,cont);
}
